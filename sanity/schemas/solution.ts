...
export default defineType({
  ...
  fields: [
    ...
    localizedText('excerpt', 'Excerpt', {
      group: 'basic',
      rows: 3,
      description: 'Short description used on listing cards.',
    }),
    defineField({
      name: 'icon',
      title: 'Icon key',
      type: 'string',
      group: 'basic',
      description: 'Optional lucide-react icon name (e.g. "Cog", "Truck").',
    }),
    defineField({
      name: 'image',
      title: 'Image',
    ...
    localizedStringArray('applications', 'Typical applications', { group: 'taxonomy' }),
    localizedStringArray('robotTypes', 'Suitable robot types', { group: 'taxonomy' }),
    defineField({
      name: 'industries',
  ...
  preview: {
    ...
    prepare({ title, subtitle, media }) {
      return { title: title || '(untitled solution)', subtitle, media }
    },
  },
  orderings: [
